...
```{r}
#Create file path
dir.create(file.path('F:/CalCurCEAS PACE/'))
dir.create(file.path('F:/CalCurCEAS PACE/metadata'))
```

```{r}
#Move GPS data
for (i in 1:nrow(dd)) {
  dir.create(file.path('F:/CalCurCEAS PACE/metadata', dd$Data_ID[i]))  
  ...
  dir.create(file.path(paste0('F:/CalCurCEAS PACE/metadata/', dd$Data_ID[i],'/gps')))
  
  newGPS <- file.path(paste0('F:/CalCurCEAS PACE/metadata/', dd$Data_ID[i],'/gps'))
...
for (i in 1:nrow(dd)) {
  # Accelerometer
    ...
    # Create folder/directory to move the list above to
    dir.create(file.path(paste0('F:/CalCurCEAS PACE/metadata/',
                                dd$Data_ID[i],'/accelerometer')))
    newAccel <- file.path(paste0('F:/CalCurCEAS PACE/metadata/',
                                dd$Data_ID[i],'/accelerometer'))
    # Copy list of files to new directory
  ...
  # Logs
    ...
    dir.create(file.path(paste0('F:/CalCurCEAS PACE/metadata/',
                                dd$Data_ID[i],'/logs')))
    newLogs <- file.path(paste0('F:/CalCurCEAS PACE/metadata/',
                                dd$Data_ID[i],'/logs'))
    # Copy list of files to new directory
  ...
  # Depth
    ...
    dir.create(file.path(paste0('F:/CalCurCEAS PACE/metadata/',
                                dd$Data_ID[i],'/depth')))
    newDepth <- file.path(paste0('F:/CalCurCEAS PACE/metadata/',
                                dd$Data_ID[i],'/depth'))
    # Copy list of files to new directory
...
# Create audio folder
dir.create(file.path('F:/CalCurCEAS PACE/audio'))

# Set up progress bar
pb <- txtProgressBar(min = 0, max = nrow(dd), style = 3, width = 50, char = "=")  

for (i in 1:nrow(dd)) {
    currentAudio <- list.files(paste0('Z:/RECORDINGS/DRIFTERS/CalCurCEAS_2024/RAW/',
                                      dd$Data_ID[i], '/WAV'),
                               pattern = "\\.wav$", full.names = T)

    # Create folder/directory to move the list above to
    newAudioDir <- dir.create(file.path(paste0('F:/CalCurCEAS PACE/audio/',
                                dd$Data_ID[i])))
  #   newAudio <- file.path(paste0('F:/CalCurCEAS PACE/audio/', dd$Data_ID[i]))
  #   # Copy list of files to new directory
